...
export type OperationIdentifier = 'H' | 'X' | 'Y' | 'Z' | 'CX' | 'CCX' | 'CZ' | 'SWAP' | 'S' | 'T' | 'RX' | 'RY' | 'RZ' | 'MEASURE';

export const getOperationSizeByIdentifier = (identifier: OperationIdentifier): number => {
    switch (identifier.toUpperCase()) {
        case "CCX": return 3;
        case "CX": case "CZ": case "SWAP": return 2;
        default: return 1;
    }
}
...
export const getOperationIconByIdentifier = (identifier: OperationIdentifier) => {
    switch (identifier.toUpperCase()) {
        case "CCX": case "CX": return Plus;
        case "SWAP": return X;
        default: return TextIcon(identifier);
    }
}