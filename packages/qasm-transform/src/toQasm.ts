...
const DEFAULT_PREAMBLE: QasmPreamble = { version: '3.0', includes: ['"stdgates.inc"'] };
 ...
 *
 * It emits no comments. The Java generator annotates its output with
 * `// Register q` and `// Layer 3`; here that would be self-defeating, because
 * the transform treats comments as content it cannot preserve — writing the file
 * would immediately make it read-only. Which also means: **a file generated by
 * the web IDE is read-only in the extension** until comments survive the round
 * trip. That is the honest state, not a bug to paper over.
 */
