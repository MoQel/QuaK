...
import type { SubcircuitOption } from '@quak/circuit-core';
 ...
 */
export function LibrarySubcircuitElement({ option, onOpen, onRemove }: Readonly<LibrarySubcircuitElementProps>) {
    ...
    const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        startOperationDrag(Math.max(option.qubitCount, 1));
    };
        ...
        <Tooltip delayDuration={DELAY_DURATION} open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <TooltipContent side="right" className="bg-bg-light text-text border shadow-xl p-3 max-w-[240px] z-[9999]">
                ...
                <div className="text-xs text-text-muted mt-1">
                    Drag to place it{onOpen ? ' · double-click to edit it' : ''}
                    {onRemove ? ' · right-click to delete' : ''}
                </div>
            </TooltipContent>
        </Tooltip>
    );
