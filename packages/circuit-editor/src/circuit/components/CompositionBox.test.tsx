...
import type { CompositeQuantumGateDto, SubcircuitOperationDto } from '@quak/circuit-core';
import { REGISTER_TYPE_QUANTUM } from '@quak/circuit-core';
import { QUBIT_HEIGHT } from '../util/layout.ts';
import type { FlatQubit } from '../util/types.ts';
...
describe('CompositionBox', () => {
     ...
     */
    type OnDragStart = (operationSize: number, grabOffset: number) => void;

    const startDrag = (onDragStart: Mock<OnDragStart>, clientY = 0) => {
        vi.useFakeTimers();
        const { container } = render(
            <CompositionBox
                operation={bell()}
    ...
    it('reports its full qubit count when a drag starts, so a move keeps every wire', () => {
        const onDragStart = vi.fn<OnDragStart>();
