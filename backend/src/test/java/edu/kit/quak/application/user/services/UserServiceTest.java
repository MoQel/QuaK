 ...
 * Tests user retrieval and ownership verification logic.
 */
...
class UserServiceTest {
    ...
    @Nested
    @DisplayName("verifyProjectOwnership Tests")
    class VerifyProjectOwnershipTests {

        @Test
        @DisplayName("Should pass when user owns the project")
        void verifyProjectOwnership_userOwnsProject_noException() {
            // Arrange
            Project project = new Project("Test Project");
            edu.kit.quak.core.filesystem.model.User filesystemUser = 
                    new edu.kit.quak.core.filesystem.model.User();
            filesystemUser.setId(testUserId);
            project.setOwner(filesystemUser);

            // Act & Assert - should not throw
            assertDoesNotThrow(() -> userService.verifyProjectOwnership(project, testUser));
        }

        @Test
        @DisplayName("Should throw FORBIDDEN when user does not own the project")
        void verifyProjectOwnership_userDoesNotOwnProject_throwsException() {
            // Arrange
            Project project = new Project("Test Project");
            edu.kit.quak.core.filesystem.model.User otherUser = 
                    new edu.kit.quak.core.filesystem.model.User();
            ...
            ResponseStatusException exception = assertThrows(ResponseStatusException.class,
                    () -> userService.verifyProjectOwnership(project, testUser));
            assertEquals(403, exception.getStatusCode().value());
        }

        @Test
        @DisplayName("Should throw FORBIDDEN when project has no owner")
        void verifyProjectOwnership_noOwner_throwsException() {
            // Arrange
            Project project = new Project("Test Project");
            // No owner set
