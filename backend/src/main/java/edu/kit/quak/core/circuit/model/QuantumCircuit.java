...
public class QuantumCircuit extends ElementWithId {
    ...
    public void moveQuantumOperation(String operationId, int targetRegisterIdx, int positionIdx) {
        ...
        var location = findOperationLocation(operationId).orElseThrow(() ->
            new IllegalArgumentException(String.format("Operation %s not found within circuit %s.", operationId, id))
        );
