 ...
 *
 * <p>
 * This class eliminates code duplication across DirectoryService and
 * FileService by extracting
 * shared logic for:
 *
 * <ul>
 ...
 * </ul>
 *
 * @param <T> the type of FileElement this service manages (File, Directory,
 *            etc.)
...
public abstract class AbstractFileElementService<T extends FileElement<T>> {
    ...
    protected AbstractFileElementService(
            FileElementContainerRepositoryDelegator delegator, ProjectRoleServicePort roleService) {
    ...
    /**
     * Checks that no sibling element in the same parent container already has the
     * given name
     * (case-insensitive). The element being renamed is excluded from the check.
     *
    ...
    protected void checkForDuplicateName(String elementId, String newName) {
        ...
        if (parent.hasChildWithName(newName, elementId)) {
            throw new IllegalArgumentException(
                    "An element with the name '" + newName + "' already exists in '" + parent.getName() + "'");
        }
    }

    /**
     * Retrieves an element by ID without authentication check. Should only be used
     * internally.
     *
