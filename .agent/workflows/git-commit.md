...
5. Apply code formatting in the backend by running `./gradlew spotlessApply` in the `backend` directory.
6. Based on the changes from steps 1-3 and the commit style analysis from step 4, generate an appropriate commit message that:
...
7. Output the generated commit message, stage all changes of this repo with `git add .`, and execute the commit command.
